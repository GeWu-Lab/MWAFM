...
from nets.diagonaled_mm_tvm import diagonaled_mm as diagonaled_mm_tvm, mask_invalid_locations
from nets.sliding_chunks import sliding_chunks_matmul_qk, sliding_chunks_matmul_pv
from nets.sliding_chunks import sliding_chunks_no_overlap_matmul_qk, sliding_chunks_no_overlap_matmul_pv
...
class MultiScaleSelfAttention(nn.Module):
    def __init__(self, num_attention_heads, 
        ...
        if self.attention_mode in ['sliding_chunks', 'sliding_chunks_no_overlap']:
            ...
            # assert self.attention_dilation == 2  # dilation is not supported

    def forward(
        self,
        hidden_states_q,  # [B, T, C]
        hidden_states_k,
        hidden_states_v,
        attention_mask=None,
        head_mask=None,
        ...
        hidden_states_q = hidden_states_q.transpose(0, 1)
        hidden_states_k = hidden_states_k.transpose(0, 1)
        hidden_states_v = hidden_states_v.transpose(0, 1)

        seq_len, bsz, embed_dim = hidden_states_q.size()

        assert embed_dim == self.embed_dim
        q = self.query(hidden_states_q)
        k = self.key(hidden_states_k)
        v = self.value(hidden_states_v)
        q /= math.sqrt(self.head_dim)

        # print("num head: ", self.num_heads)
        # print("head dim: ", self.head_dim)   # int(hidden_size / num_attention_heads)

        q = q.view(seq_len, bsz, self.num_heads, self.head_dim).transpose(0, 1)
        k = k.view(seq_len, bsz, self.num_heads, self.head_dim).transpose(0, 1)

        # print("long q: ", q.shape)
        # print("long k: ", k.shape)
        # print("long v: ", v.shape)

        # attn_weights = (bsz, seq_len, self.num_heads, window*2+1)
        ...
        if remove_from_windowed_attention_mask is not None:
            ...
            # remove_from_windowed_attention_mask = remove_from_windowed_attention_mask.unsqueeze(dim=-1)
            # print("remove_from_windowed_attention_mask: ", remove_from_windowed_attention_mask.shape)
        ...
        attn = attn.type_as(hidden_states_q)    # 将attn类型转为hidden_states类型
        ...
        if extra_attention_mask is not None:
            ...
            selected_hidden_states[selection_padding_mask_nonzeros[::-1]] = hidden_states_q[extra_attention_mask_nonzeros[::-1]]

            q = self.query_global(selected_hidden_states)
            k = self.key_global(hidden_states_k)
            v = self.value_global(hidden_states_v)
            q /= math.sqrt(self.head_dim)
            ...
            attn[extra_attention_mask_nonzeros[::-1]] = nonzero_selected_attn.view(len(selection_padding_mask_nonzeros[0]), -1).type_as(hidden_states_q)
...
#     model = MultiScaleSelfAttention(config, layer_id)