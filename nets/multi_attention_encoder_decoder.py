    ...
    def __init__(self, config):
        ...
        if config.attention_mode == 'n2':
            ...
            for i, layer in enumerate(self.model.encoder.layers):
                layer.self_attn = MultiScaleSelfAttentionForBart(config, layer_id=i)
...
class MultiScaleSelfAttentionForBart(nn.Module):
    def __init__(self, config, layer_id):
        super().__init__()
        self.embed_dim = config.d_model
        self.multi_scale_self_attn = MultiScaleSelfAttention(config, layer_id=layer_id)
    ...
    def forward(
        ...
        outputs = self.multi_scale_self_attn(
            query.transpose(0, 1),  # MultiScaleSelfAttention expects (bsz, seqlen, embd_dim)
